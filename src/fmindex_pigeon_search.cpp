...
std::vector<std::vector<seqan3::dna5>> cut_query(int pieces, std::vector<seqan3::dna5> const & query)
{
    ...
    for (int i = 0; i < pieces; ++i)
    {
        if (i == pieces - 1)
            ...
            pieced_queries.emplace_back(query.begin() + l * i, query.begin() + l * (i + 1));
    }
...
bool verify(std::vector<seqan3::dna5> const & ref,
            ...
            size_t max_errors)
{
    if (pos + query.size() > ref.size()) return false;

    size_t errors = 0;
    for (size_t i = 0; i < query.size(); ++i)
    {
        if (ref[pos + i] != query[i] && ++errors > max_errors)
            return false;
    }
    ...
    for (auto const & query : queries)
    {
        ...
        for (auto & part : parts)
        {
            for (auto const & hit : seqan3::search(part, index, cfg))
            {
                ...
                if (ref_id >= reference.size()) continue;

                size_t candidate = hit.reference_begin_position();
                if (candidate < offset) continue;
                candidate -= offset;

                if (verify(reference[ref_id], query, candidate, number_of_errors))
                    hits.emplace_back(ref_id, candidate);
            }
